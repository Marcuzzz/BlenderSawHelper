    ...
    if makematerial:
        ...
        if principled_bsdf is not None:
            principled_bsdf.inputs[
                "Base Color"
            ].default_value = color  # set the color to red
...
def initBoundries(width, length, height):
    ...
    colors = np.column_stack(
        (colors_without_alpha, np.ones(colors_without_alpha.shape[0]))
    )
    ...
    plank = createPlank(
        "plank01", (blength, bwidth, height), tuple(colors[0]), z=-(height * 2)
    )
    removefrom = plank.users_collection
    coll.objects.link(plank)
    removefrom[0].objects.unlink(plank)
    ...
    plank = createPlank(
        "plank02", (length, width, height), tuple(colors[1]), z=-(height)
    )
    removefrom = plank.users_collection
    coll.objects.link(plank)
    removefrom[0].objects.unlink(plank)
...
class MySawHelper_Operator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            createPlanks(
                props.length, props.width, props.height, props.cut, props.objname
            )
...
classes = [MySawHelper_Operator, MySawHelper_custom_menu, MySawHelper_Props]
